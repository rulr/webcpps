...
namespace webcpp {
	...
	const int subsystem::DEFAULT_PORT = 8889;
	const int subsystem::DEFAULT_MAX_QUEUED = 100;
	const int subsystem::DEFAULT_MAX_THREADS = 50;
	const std::string subsystem::DEFAULT_SOFTWARE_VERSION = "webcpps/0.1.1";
	...
	void subsystem::initialize(Poco::Util::Application& app) {
		...
		server = new Poco::Net::HTTPServer(new webcpp::factory(serverConf), port, pars);
		server->start();
	}
